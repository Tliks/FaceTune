...
## [Unreleased]
### Added
- アニメーション式のリップシンク設定を追加しました
  - 任意のブレンドシェイプの組み合わせで好きな口の形状が設定可能になります。
  - また、一括設定の他にも表情単位で設定できるため、表情と干渉する場合でも個別に最終的な形状が設定可能になります。
- ヒエラルキーで選択している表情に対するまばたきやリップシンクのプレビューを追加しました。
- マルチフレームアニメーションに対するシークバーを用いたプレビューを追加しました。
- Modular Avatar Informationなどで使用パラメータ数が見れるようになりました。 
- まばたきやリップシンク専用の編集画面が追加されました。
- `FaceTune Avatr Control`に`AFK対応`を追加しました。
  - 従来、AFKモーション再生中に表情が上書きされていた問題が解消されます。
  - `MMD対応`同様、この設定がない場合デフォルトではAFKやMMDは考慮されません。
  - 既存ユーザーは手動でコンポーネントを追加してください。一方新規テンプレートでは最初から追加されます。

### Changed
- `FaceTune Expression Data`を`FaceTune Data`へと改名し、仕様を大幅に変更しました
- `FaceTune Data`にまばたきやリップシンクなどの設定を追加し、表情内容全体を含められるようにしました。
- `FaceTune`に、表情内容を参照するモードを追加し、`FaceTune Data`などを参照できるようにしました。
  - これにより、`FaceTune Data`を用いて表情内容を複数作成し、`FaceTune`では条件を設定した上でDataの中から好きなものを参照する、表情内容全体を取り回せる構造が作成可能になりました。
- 親の`FaceTune`へ`FaceTune Data`の表情・表情外アニメーションが暗黙的に合成される仕様を廃止し、代わりに`FaceTune`に、複数の表情を合成する明示的なモードが追加されました。
- プレビューの仕様を一部変更しました。
- UIを改善しました。
...
- メニューの配置順序を修正。
- 一部のケースでMMD再生中にブレンドシェイプが3倍となる問題を修正。
